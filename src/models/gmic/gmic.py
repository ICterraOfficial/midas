...
See original repository here: https://github.com/nyukat/GMIC .
"""
...
class GMIC(nn.Module):
    ...
    def _convert_crop_position(self, crops_x_small, cam_size, x_original):
        ...
        top_k_prop_x = torch.Tensor(top_k_prop_x)
        top_k_prop_y = torch.Tensor(top_k_prop_y)

        top_k_interpolate_x = torch.round(top_k_prop_x * H).unsqueeze(-1)
        top_k_interpolate_y = torch.round(top_k_prop_y * W).unsqueeze(-1)
        top_k_interpolate_2d = torch.cat([top_k_interpolate_x, top_k_interpolate_y], axis=-1)
    ...
    def _retrieve_crop(self, x_original_pytorch, crop_positions, crop_method):
        ...
        if 'cuda' in self.experiment_parameters['device_type']:
    ...
    def forward(self, x_original):
        ...
        self.concat_vec = torch.cat([global_vec, z], dim=1)
        self.y_fusion = torch.sigmoid(self.fusion_dnn(self.concat_vec))

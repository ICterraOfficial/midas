...
class GlobalNetwork(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        # downsampling-branch
        if parameters['global_model'] =='resnet18':
            self.downsampling_branch = DownsampleNetworkResNet18V1()
        elif 'efficientnet' in parameters['global_model']:
            self.downsampling_branch = DownsampleNetworkEfficientNet_B0()
        else:
...
class TopTPercentAggregationFunction(AbstractMILUnit):
    ...
    def forward(self, cam):
        ...
        top_t = int(np.round(W*H*self.percent_t))
...
class RetrieveROIModule(AbstractMILUnit):
    ...
    def forward(self, x_original, cam_size, h_small):
        ...
        for _ in range(self.num_crops_per_class):
            ...
            mask = tools.generate_mask_uplft(current_images, crop_shape_adjusted, max_pos, self.device)
            current_images = current_images.to(self.device)
            current_images = current_images * mask

        all_max_position = [max_position.to(self.device) for max_position in all_max_position]

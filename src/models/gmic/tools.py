...
def generate_mask_uplft(input_image, window_shape, upper_left_points, device):
    """
    ...
    if 'cuda' in device:
        mask_x = mask_x.to(device)#.cuda()
        mask_x_min = mask_x_min.to(device)#.cuda()
        mask_x_max = mask_x_max.to(device)#.cuda()
        mask_y = mask_y.to(device)#.cuda()
        mask_y_min = mask_y_min.to(device)#.cuda()
        mask_y_max = mask_y_max.to(device)#.cuda()
